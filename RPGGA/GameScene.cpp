...
void GameScene::ChangeItems(Entity* inventoryItem , Entity* equipedItem)
{
	...
	for (int it = 0; it < 6; it++) {
		ofsEquiped
			...
			<< std::endl;
		//std::cout << player->getEquipedItems()->getItemById(it)->getName() << std::endl;
	}
	ofsEquiped.close();
	...
	for (int iz = 0; iz < 6; iz++) {
		ofsInventory << this->inventory->getItemById(iz)->getName()
			<< " " << this->inventory->getItemById(iz)->getHp()
			<< " " << this->inventory->getItemById(iz)->getPower()
			<< " " << this->inventory->getItemById(iz)->getType()
			<< std::endl;
		//std::cout << inventory->getItemById(iz)->getName() << std::endl;
